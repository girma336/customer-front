...
import { useAuth, useGetRQuery } from '../../context/AutoProvider';
...
const Login: React.FC = () => {
  ...
  // const [customer, setCutomer] = useState<any>({})
  const { user, login } = useAuth();
  const navigate = useNavigate()
  

    const { data:customer,  isError: error } =  useGetRQuery(token)

  const { mutate, data, isPending, isError, isSuccess } = useMutation({
    mutationFn: (userData: UserData) => 
       fetch("http://localhost:3000/api/v1/customer/login", {
        method: "POST",
      ...
      onSuccess: (response) => {
        if (response.status === 'success') {
          ...
          login(customer.data, response.api_token)
            navigate('/')
        }else  {
  ...
  return (
    <div className="container mx-auto p-4 w-[350px] sm:w-[500px] rounded-xl mt-10 border border-gray-500">
      <ToastContainer />
      <h1 className='mb-4 text-2xl bg-gradient-to-r from-yellow-400 via-red-500 to-pink-500 text-white p-3'>Login</h1>
      {user && (
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className='p-4'>If you have not an account <span className='text-[#8b5cf6] ml-2'><Link to="/signup">Signup</Link></span></div>
